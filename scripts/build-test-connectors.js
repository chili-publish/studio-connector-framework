...
fs.readdir(baseDir, { withFileTypes: true }, (err, files) => {
    ...
    files.forEach((file) => {
        if (file.isDirectory()) {
            ...
            const testCommand = spawnSync('yarn',["test"], { cwd: dirPath, stdio: 'inherit' });
...
function trackProcess(command, dirPath, type) {
    if (command.error) {
        console.error(`Failed to start ${type} command in ${dirPath}:\n${command.error}`);
    }

    console.log(`${type} command finished with exit code ${command.status} in ${dirPath}`);
}
