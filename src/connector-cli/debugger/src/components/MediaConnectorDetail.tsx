...
const MediaConnectorDetail: React.FC = () => {
    ...
    const handleMetadataSubmit = () => {
        dispatch({ type: 'ADD_METADATA', payload: { key: metadataKey, value: metadataValue } });
        setMetadataKey('');
        setMetadataValue('');
    };
    ...
    async function executeConnectorDetail(): Promise<void> {
        ...
        const results = await connector.detail(state.detailOptions.id, state.metadata);
    ...
    return (
        <>
            <Form onFinish={handleDetailOptionsSubmit} labelCol={{ span: 8 }}
                wrapperCol={{ span: 16 }}
                style={{ maxWidth: 600 }}
            ...
            <Form onFinish={handleMetadataSubmit} labelCol={{ span: 8 }}
                wrapperCol={{ span: 16 }}
                style={{ maxWidth: 600 }}
                initialValues={{ remember: true }}>
                <Form.Item label="Metadata Key">
                    <Input value={metadataKey} onChange={e => setMetadataKey(e.target.value)} />
                </Form.Item>
                <Form.Item label="Metadata Value">
                    <Input value={metadataValue} onChange={e => setMetadataValue(e.target.value)} />
                </Form.Item>
