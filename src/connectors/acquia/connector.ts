...
export default class AcquiaConnector implements Media.MediaConnector{
    ...
    async query(options: Connector.QueryOptions, context: Connector.Dictionary): Promise<Media.MediaPage> {
        try {
            ...
            url = url + `api/rest/asset/search/query?start=${startIndex}&max=${options.pageSize}&sort=date-added-reversed&searchdocuments=false`;
            ...
            if (!t?.ok) {
                this.runtime.logError(`Error while querying Acquia DAM: ${t?.status} - ${t?.statusText}`);
                return {
                    pageSize: 0,
                    data: [],
                    links: {
