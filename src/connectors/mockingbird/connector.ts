...
export const PLACEHOLDER_PNGS: Record<string, string> = {
  ...
  mountain:
    'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR42mOwbvoGAAKvAbSJsn7VAAAAAElFTkSuQmCC',
  ...
  ocean:
    'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR42mPgW/oSAAJhAZ13CQERAAAAAElFTkSuQmCC',
  desert:
    'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR42mN4tZkDAAQwAaYb1KNdAAAAAElFTkSuQmCC',
  snow: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR42mM4ffUhAATwAoLs9l8mAAAAAElFTkSuQmCC',
  tropical:
    'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR42mMQ2bEMAAJWAXPppz4uAAAAAElFTkSuQmCC',
  abstract:
    'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR42mN44zETAAPxAc6x3OsqAAAAAElFTkSuQmCC',
  vintage:
    'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR42mOoKMwBAAK6AVbzBxfJAAAAAElFTkSuQmCC',
};
...
export default class MockingbirdConnector implements Media.MediaConnector {
  ...
  async query(
    ...
    return {
      ...
      data,
      links: {
        nextPage: nextOffset < MOCK_ASSETS.length ? String(nextOffset) : '',
      },
    };
  }
  ...
  async detail(
    ...
    const asset =
      MOCK_ASSETS.find((a) => a.id === id) ??
      MOCK_ASSETS[Math.floor(Math.random() * MOCK_ASSETS.length)];
    return {
      id: asset.id,
      name: asset.name,
      relativePath: '/',
  ...
  async download(
    ...
    if (this.runtime.options['useRemoteImages'] === 'true') {
      ...
      if (previewType === 'thumbnail') {
        w = 200;
        h = Math.round((200 * h) / w);
      } else if (previewType === 'mediumres') {
        w = 400;
        h = Math.round((400 * h) / w);
      }
      const resp = await this.runtime.fetch(
        `https://picsum.photos/seed/${asset.id}/${w}/${h}`,
        { method: 'GET' }
      );
      if (!resp.ok)
        throw new ConnectorHttpError(
          resp.status,
          `picsum returned ${resp.status}`
        );
      return resp.arrayBuffer;
    }

    const base64 =
      PLACEHOLDER_PNGS[asset.placeholder] ?? PLACEHOLDER_PNGS['sunset'];
    const resp = await this.runtime.fetch(`data:image/png;base64,${base64}`, {
      method: 'GET',
    });
    return resp.arrayBuffer;
  }
