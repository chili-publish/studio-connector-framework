...
const FILLERS = [
  ...
  'with extra bubbles',
  'near the mock dock',
  'before the tide compiles',
  'in a deeply questionable current',
  'without any production guarantees',
  'like a confused cephalopod',
  'during light panic',
...
const LONGER_TEMPLATES = [
  () =>
    `The ${pickRandom(ADJECTIVES)} ${pickRandom(NOUNS)} ${pickRandom(
      VERBS
    )} ${pickRandom(FILLERS)}.`,
  () =>
    `Meanwhile, a ${pickRandom(ADJECTIVES)} ${pickRandom(
      NOUNS
    )} quietly ${pickRandom(VERBS)} near the testing reef.`,
  () =>
    `No ${pickRandom(NOUNS)} was harmed, although the ${pickRandom(
      ADJECTIVES
    )} current remains highly suspicious.`,
  () =>
    `Mocktopus reports that the ${pickRandom(NOUNS)} ${pickRandom(
      VERBS
    )} again, which is technically expected behavior.`,
  () =>
    `The ${pickRandom(ADJECTIVES)} ${pickRandom(NOUNS)} ${pickRandom(
      VERBS
    )} ${pickRandom(FILLERS)}, while a ${pickRandom(ADJECTIVES)} ${pickRandom(
      NOUNS
    )} quietly reorganizes the kelp-based backlog for testing purposes only.`,
  () =>
    `Somewhere beyond the ${pickRandom(ADJECTIVES)} reef, the ${pickRandom(
      NOUNS
    )} ${pickRandom(
      VERBS
    )} with impressive confidence, even though the surrounding ${pickRandom(
      NOUNS
    )} community agrees that nothing about the current situation appears production-ready.`,
  () =>
    `At approximately bubble o'clock, a ${pickRandom(ADJECTIVES)} ${pickRandom(
      NOUNS
    )} was observed to ${pickRandom(VERBS)} ${pickRandom(
      FILLERS
    )}, prompting several nearby ${pickRandom(
      NOUNS
    )} to file strongly worded but entirely decorative incident reports.`,
  () =>
    `Mocktopus analytics indicate that the ${pickRandom(
      ADJECTIVES
    )} ${pickRandom(NOUNS)} continues to ${pickRandom(
      VERBS
    )} near the outer coral ring, which is technically acceptable, emotionally suspicious, and fully aligned with the expected chaos profile.`,
  () =>
    `Although no certified ${pickRandom(
      NOUNS
    )} experts were available for comment, the ${pickRandom(
      ADJECTIVES
    )} tide insists that something definitely ${pickRandom(VERBS)} ${pickRandom(
      FILLERS
    )}, leaving only bubbles, questions, and a mildly offended ${pickRandom(
      NOUNS
    )} behind.`,
  () =>
    `Following a comprehensive review of the ${pickRandom(
      ADJECTIVES
    )} seabed, the local ${pickRandom(
      NOUNS
    )} committee concluded that repeated ${pickRandom(
      VERBS
    )} events are normal during low tide, especially when mock data has been seasoned with extra ink and unnecessary enthusiasm.`,
  () =>
    `The ${pickRandom(ADJECTIVES)} ${pickRandom(
      NOUNS
    )} drifted into view shortly before sunset, began to ${pickRandom(
      VERBS
    )} ${pickRandom(
      FILLERS
    )}, and somehow turned an otherwise routine underwater operation into a deeply questionable but visually impressive demonstration.`,
  () =>
    `Despite earlier assurances from the Department of ${pickRandom(
      ADJECTIVES
    )} ${pickRandom(NOUNS)}, the system once again ${pickRandom(
      VERBS
    )} under gentle oceanic pressure, confirming that all observed weirdness remains reproducible, testable, and oddly adorable.`,
];
...
const createMockSentence = (): string => {
  ...
  return `${selectAdjectives(1)} ${pickRandom(NOUNS)} ${pickRandom(
    VERBS
  )} ${pickRandom(FILLERS)}.`;
};
...
function parseDSL(
  ...
  for (const entry of splitFields(schema)) {
    ...
    if (colonIndex === -1) {
      logError(
        `Mocktopus: invalid field "${trimmed}" — expected "fieldName:type" or "fieldName:type(param=value)"`
      );
      continue;
    }
    ...
    if (!name) {
      logError(
        `Mocktopus: invalid field "${trimmed}" — field name is required`
      );
      continue;
    }
    ...
    if (
      parenIndex !== -1 &&
      (closeParenIndex === -1 || closeParenIndex < parenIndex)
    ) {
      logError(`Mocktopus: invalid field "${trimmed}" — missing closing ")"`);
      continue;
    }
    const paramsStr =
      parenIndex === -1 ? '' : rest.slice(parenIndex + 1, closeParenIndex);
    if (!isFieldType(rawType)) {
      logError(
        `Mocktopus: unknown type "${rawType}" for field "${name}" — valid types: ${VALID_TYPES.join(
          ', '
        )}`
      );
      continue;
...
function generateValue(
  ...
  switch (field.type) {
    case 'shortText':
      return createMockWords(
        ParseNumberParameter(field.params.numberOfWords, 2)
      );
    case 'longText':
      return createMockParagraphs(
        ParseNumberParameter(field.params.numberOfParagraphs, 2)
      );
    case 'number': {
...
export default class MocktopusConnector implements Data.DataConnector {
    ...
    return [
      {
        name: 'schema',
        displayName:
          'Schema (e.g. firstName:shortText, age:number, photo:image)',
        type: 'text',
      },
      { name: 'recordCount', displayName: 'Record count', type: 'text' },
      {
        name: 'simulateDelays',
        displayName: 'Simulate delays',
        type: 'boolean',
      },
  ...
  async getPage(
    ...
    const schema = (context['schema'] as string) ?? '';
    const recordCount =
      parseInt((context['recordCount'] as string) ?? '10', 10) || 10;
    const simulateDelays = (context['simulateDelays'] as boolean) ?? false;
    const minDelay =
      parseInt((context['minDelay'] as string) ?? '100', 10) || 100;
    const maxDelay =
      parseInt((context['maxDelay'] as string) ?? '1000', 10) || 1000;
    ...
    const offset = config.continuationToken
      ? parseInt(config.continuationToken, 10)
      : 0;
    ...
    const continuationToken =
      nextOffset < recordCount ? String(nextOffset) : undefined;
